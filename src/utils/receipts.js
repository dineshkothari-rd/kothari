...
const getReceiptAmount = (payment) =>
  Number(payment.amountPaid ?? payment.amount ?? 0) || 0;

const getReceiptTotal = (payment) => Number(payment.totalRent) || 0;

const getReceiptBalance = (payment) =>
  Math.max(
    0,
    Number(payment.balance ?? getReceiptTotal(payment) - getReceiptAmount(payment)) ||
      0,
  );
...
export function getReceiptNumber(payment) {
  return `RCPT-${payment.month || "NA"}-${String(payment.id || Date.now())
    .slice(0, 8)
    .toUpperCase()}`;
}
...
export function buildReceiptText(payment) {
  ...
  return [
    ...
    `${type.feeLabel}: ${formatCurrency(getReceiptTotal(payment))}`,
    `Paid: ${formatCurrency(getReceiptAmount(payment))}`,
    `Balance: ${formatCurrency(getReceiptBalance(payment))}`,
  ...
  <body>
    <main class="page">
      ...
      <section class="content">
        ...
        <div class="total">
          <div class="row"><span>${safeText(type.feeLabel)}</span><strong>${formatCurrency(getReceiptTotal(payment))}</strong></div>
          <div class="row paid"><span>Amount Paid</span><strong>${formatCurrency(getReceiptAmount(payment))}</strong></div>
          <div class="row due"><span>Balance</span><strong>${formatCurrency(getReceiptBalance(payment))}</strong></div>
        </div>
      </section>
      <footer class="footer">Generated by Reamesh Kothari.</footer>
    </main>
  </body>
</html>`;
}
